...
const Checkout = () => {
  ...
  const applyPromoCode = () => {
    if (promoCode.toLowerCase() === "PICKANDDROP") {
      setDiscount(10);
      setShowPopper(true);
...
//     verifyPickAndDropPayment(orderId, amount, token),
//   onSuccess: () => {
 ...
 const confirmOrderMutation = useMutation({
  ...
  onSuccess: (data) => {
    ...
    if (paymentMode === "cash") {
      ...
      if (!orderId || !amount) {
        return alert("⚠️ Missing payment data.");
      }

      verifyPickAndDropPayment(orderId, amount, token); // Just call it here
    }
  },
});
  ...
  //   } else {
  //     if (!orderId || !amount) {
  //       alert("⚠️ No payment details returned.");
  //       return;
  //     }

  //     verifyPaymentMutation.mutate({ orderId, amount });
  //   }
  // }
